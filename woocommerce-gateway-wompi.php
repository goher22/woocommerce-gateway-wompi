<?php
/*
Plugin Name: Wompi Plugin para WooCommerce Panama
Plugin URI: https://docs.wompi.pa/
Description: Plugin WooCommerce para la pasarela de pagos Wompi Panama.
Version: 1.0.0
Author: Wompi
...
function woocommerce_gateway_wompi_wc_missing_notice()
{
    echo '<div class="error"><p><strong>' .
        sprintf(
            esc_html__(
                "WooCommerce Wompi Gateway requires WooCommerce to be installed and active. You can download %s here.",
...
function woocommerce_gateway_wompi_wc_not_supported_notice()
{
    echo '<div class="error"><p><strong>' .
        sprintf(
            esc_html__(
                'WooCommerce Wompi Gateway requires WooCommerce %1$s or greater.',
                "woocommerce-gateway-wompi"
            ),
            WC_WOMPI_MIN_WC_VER,
            WC_VERSION
...
function woocommerce_gateway_wompi_init()
{
    ...
    if (!class_exists("WooCommerce")) {
        add_action(
            "admin_notices",
            "woocommerce_gateway_wompi_wc_missing_notice"
        );
        return;
    }
    ...
    if (version_compare(WC_VERSION, WC_WOMPI_MIN_WC_VER, "<")) {
        add_action(
            "admin_notices",
            "woocommerce_gateway_wompi_wc_not_supported_notice"
        );
        return;
    }
